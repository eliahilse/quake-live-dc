...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('stats')
        .setDescription('View stats for a specific Steam ID or a linked Discord user')
        .addStringOption(option =>
            option.setName('steamid')
                .setDescription('The Steam ID to view stats for'))
        .addUserOption(option =>
            option.setName('user')
                .setDescription('The Discord user to view stats for')),

    async execute(interaction, keyv) {
        let steamId;
        // Check if a Steam ID was provided directly
        if (interaction.options.getString('steamid')) {
            steamId = interaction.options.getString('steamid');
        } else if (interaction.options.getUser('user')) { // Check if a Discord user was mentioned
            // Retrieve the mentioned user's linked Steam ID from the key-value store
            const mentionedUser = interaction.options.getUser('user');
            steamId = await keyv.get(mentionedUser.id);
            // Check if the mentioned user has linked their Steam ID
            if (!steamId) {
                await interaction.reply({ content: `${mentionedUser.username} has not linked their Steam ID.`, ephemeral: true });
                return;
            }
        } else {
            await interaction.reply({ content: 'Please provide a Steam ID or mention a Discord user.', ephemeral: true });
            return;
        }
